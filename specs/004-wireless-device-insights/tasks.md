...
- [ ] T013 先写 manager 无线发现契约失败测试，覆盖 generation/session 归属、取消、覆盖 source create/start 的超时与取消传播、create 返回与取消竞争的 source 交接清理、source 启动同步拒绝与异步失败的结构化错误、关闭完成前的并发发现拒绝和关闭清理于 `core/adb/src/test/kotlin/com/sheen/adb/core/internal/WirelessSessionManagerContractTest.kt`
