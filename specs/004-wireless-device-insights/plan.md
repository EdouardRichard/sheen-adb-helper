...
- 应用 UID 接入拆为两个实施边界：先仅在 `ApplicationCapabilities.kt` 与 `AdbModels.kt` 增加 `pm list packages -3 -U --user` 的可选 UID 解析、规范化模型和兼容默认值；随后单独修改 `DefaultAdbSessionManager.kt`，把同一次当前用户第三方应用查询得到的 UID 写入 Session 快照。这样既有应用操作仍消费同一快照，又保持每任务最多修改两个文件。
