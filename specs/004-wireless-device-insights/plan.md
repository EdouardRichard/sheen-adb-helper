...
- T074 首轮 lint 暴露库模块静态分析无法从 App merged Manifest 推导 `ACCESS_NETWORK_STATE`，也无法从注入的 `apiLevel` 推导 Android 13 extension 3 / Android 14 extension 7 的平台可用性。新增 T073A 只在 `AndroidNsdDiscoveryAdapter.kt` 的五个已由 Manifest、`apiLevel` 分支和 `SecurityException` 映射覆盖的平台调用边界使用窄范围 lint suppression；不扩大权限、不降低运行时 guard，也不创建 lint baseline。修复后必须先通过 core discovery tests 与 `:core:adb:lintDebug`，再重新开始 T074 全量验证。
