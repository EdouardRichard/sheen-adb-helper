...
Kadb Android 运行时依赖包括 Okio、Kotlin 标准库、Coroutines、AndroidX DocumentFile、Bouncy Castle、`spake2-java` 与 HiddenApiBypass。原审查将其全部判定为项目白名单许可证；2026-07-15 结项复核确认 `spake2-java 1.0.5` 的发布 POM 与精确标签 `LICENSE` 实为 GPL-3.0-or-later，原判定已失效，详见 ADR 0004。Kadb 会增加密码学与协议代码体积；Phase 0 优先验证协议可行性，不把未使用的文件管理等能力暴露到项目 API。Release 前必须测量 APK/R8 结果并考虑裁剪或替换。
...
- 项目负责人已确认 v0.01 在 HyperOS 3 / Android 16 基线上的总体功能真机验收通过，因此“当前没有该基线真机证据”不再作为总体功能结论；逐项结果和被验收包映射仍按 [`../archive/releases/v0.01/结项记录.md`](../archive/releases/v0.01/结项记录.md) 列为待补证据。
- `spake2-java 1.0.5` 的许可证冲突阻断发布合规结项。在 ADR 0004 完成前，本 ADR 不得据此状态宣称依赖合规已经通过。
